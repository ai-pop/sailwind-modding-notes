...
> **Обновление:** точная декомпиляция `Crest.dll` теперь опубликована в [заметке 71](71-crest-simplefloatingobject-exact-model.md). Старые рассуждения ниже о том, «работает ли» `SimpleFloatingObject`, сохранены как история исследования; для реальной формулы, `ForceMode.Acceleration`, кубического подъёма и `_raiseObject` используйте заметку 71.
