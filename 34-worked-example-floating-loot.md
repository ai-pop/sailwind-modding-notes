...
1. **Префаб** `ShipItem` + `SaveablePrefab` (+ валидный индекс).
2. **Позиция**: думать в реальных координатах, спавнить в координатах сцены (`±outCurrentOffset`), Y — от `OceanHeight` или с опорой на `SimpleFloatingObject`.
3. **Ритуал**: `sold=true` (+ `RegisterAsMissionless`), `RegisterToSave`.
4. **Всё остальное** (подбор, плавание, физика, сохранение, UI-подсказка) даёт игра.
