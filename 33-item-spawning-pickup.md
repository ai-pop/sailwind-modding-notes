...
| Механика | Детали |
|----------|--------|
| `containedPrefab` | Префаб предмета внутри. |
| `amount` | Число предметов; `> 0` = запечатан. |
| `UnsealCrate()` | Спавнит `amount` копий содержимого (на высоте +100.5, падают сверху), регистрирует в сейв, для еды применяет `dried/smoked`, кладёт в `CrateInventory`, звук вскрытия, открывает UI. |
| `OnLoad()` | `value = containedValue * amount + 20` (× 1.2 если `smokedFood`). |
| `OnAltActivate()` | ПКМ: если запечатан → `CrateSealUI` (вскрыть); если пуст → открыть инвентарь ящика. |
| `lookText` | `"name (amount)"` или `"crate"`; для миссионных — текст миссии. |
...
5. **Ящик** (`ShipItemCrate`) — готовый «лут-контейнер»: запечатан (`amount>0`), вскрытие спавнит содержимое.
